...
/checklist
  → /chat?checklist=BASE64&nickname=URL_ENCODED
    → /result?checklist=BASE64&chat=BASE64&nickname=URL_ENCODED
```

- `nickname`: 체크리스트 닉네임 입력 → chat API 시스템 프롬프트에 삽입 + result 커버 페이지 표시에 사용
...
### 커버 페이지 (결과 진입 시 첫 화면)
분석 완료 후 결과를 바로 보여주지 않고 커버 페이지를 먼저 표시.

- 구현: `app/result/page.tsx` `showCover` 상태 (기본 `true`, 버튼 클릭 시 `false`)
- 표시 내용:
  - 닉네임 + "님의 결혼 파트너 성향"
  - 파트너 탐색 유형 배지 (이모지 + 유형명)
  - 체크리스트 완료 ✓ / 행복이 대화 완료 ✓ 확인 배지
  - "결과 보기 →" 버튼
...
| 항목 | 내용 |
|------|------|
| 파트너 탐색 유형 | 4유형 중 AI가 선택한 유형 + 유형 설명 1~2문장 |
...
AI가 체크리스트 + 채팅 대화 전체를 보고 다음 4가지 중 하나로 분류:

| 유형명 | 이모지 | 핵심 |
|--------|--------|------|
| 안정탐색형 | 🏛️ | 경제력·신뢰를 최우선으로 하는 파트너 추구 |
| 감성교류형 | 💬 | 깊은 대화·정서 공유를 추구하는 파트너십 |
| 독립동반자형 | 🌿 | 개인 자율성을 유지하는 동등한 파트너십 |
| 가정헌신형 | 🏡 | 가정·안정·헌신을 중심에 두는 파트너십 |
...
```json
{
  "psychInsight": {
    ...
    "typeDescription": "이 유형으로 분류한 이유 (1~2문장)"
  }
}
```
